...
// https://datasheet.lcsc.com/lcsc/1811141211_Bosch-Sensortec-BME680_C125972.pdf
...
// Variables to hold sensor state and current iaq state determined via above data
struct sensor_value temp, press, humidity, iaq;
SensorIaqState sensor_iaq_state = SensorIaqState::Good;

// Display device
const struct device *display_dev;
// Label for updating state
lv_obj_t *display_label;

static bool filtration_active = false;
static const struct pwm_dt_spec fan_pwm = PWM_DT_SPEC_GET(DT_ALIAS(pwm_led0));
...
K_TIMER_DEFINE(sensor_read_timer, AppTask::SensorReadTimer, NULL);
...
SensorIaqState getIaqState(int iaqScore) {
    if (iaqScore <= 50) return SensorIaqState::Good;
    else if (iaqScore <= 100) return SensorIaqState::Average;
    else if (iaqScore <= 150) return SensorIaqState::LittleBad;
    else if (iaqScore <= 200) return SensorIaqState::Bad;
    else if (iaqScore <= 300) return SensorIaqState::Worse;
    else return SensorIaqState::VeryBad;
}
...
CHIP_ERROR AppTask::Init()
{
	...
	// Initialize LVGL label for displaying sensor state
	display_label = lv_label_create(lv_scr_act());
	lv_obj_align(display_label, LV_ALIGN_CENTER, 0, 0);
    lv_label_set_text(display_label, "T: 0C - P: 0\nH: 0% - IAQ: 0\n");
    lv_task_handler();
	...
	// Start a timer for reading sensor data
    k_timer_start(&sensor_read_timer, K_MSEC(3000), K_MSEC(3000));
...
void AppTask::ReadSensorData(const AppEvent &event) {
	// Find BME680 device
    const struct device *bme680_sensor = DEVICE_DT_GET_ONE(bosch_bme680);

	// Betch BME680 sensor data
    sensor_sample_fetch(bme680_sensor);
    sensor_channel_get(bme680_sensor, SENSOR_CHAN_AMBIENT_TEMP, &temp);
    sensor_channel_get(bme680_sensor, SENSOR_CHAN_PRESS, &press);
    sensor_channel_get(bme680_sensor, SENSOR_CHAN_HUMIDITY, &humidity);
	// Given BSEC library SENSOR_CHAN_IAQ
    sensor_channel_get(bme680_sensor, (sensor_channel) SENSOR_CHAN_IAQ, &iaq);

	SensorIaqState previousSensorIaqState = sensor_iaq_state;
	sensor_iaq_state = getIaqState(iaq.val1);

    AppEvent update_event;
    update_event.Type = AppEventType::DisplayUpdate;
	update_event.Handler = UpdateDisplay;
    PostEvent(update_event);
	
	if (previousSensorIaqState != sensor_iaq_state) {
		switch (sensor_iaq_state) {
			case SensorIaqState::Good:
			case SensorIaqState::Average:
			if (filtration_active) {
				// Disable filtration
				filtration_active = false;
				AppEvent event;
				event.Type = AppEventType::FiltrationDeactivate;
				event.Handler = DeactivateFiltration;
				PostEvent(event);
			}
			break;
			default: 
				if (!filtration_active) {
					...
					AppEvent event;
					event.Type = AppEventType::FiltrationActivate;
					event.Handler = ActivateFiltration;
					PostEvent(event);
				}
			break;
		}
...
void AppTask::ActivateFiltration(const AppEvent &event) {
	// Set fan to 30% which is plenty for this application
	pwm_set_dt(&fan_pwm, MAX_PERIOD, MAX_PERIOD * 30 / 100);
...
void AppTask::DeactivateFiltration(const AppEvent &event) {
	// Set fan to 0%
	pwm_set_dt(&fan_pwm, MAX_PERIOD, 0);
...
void AppTask::UpdateDisplay(const AppEvent &event) {
    lv_label_set_text_fmt(display_label, "T: %dC - P: %d\nH: %d%% - IAQ: %d\n",
                          temp.val1, press.val1, humidity.val1, iaq.val1);
    lv_task_handler();
}

void AppTask::SensorReadTimer(struct k_timer *timer_id) {
    AppEvent event;
    event.Type = AppEventType::SensorDataRead;
	event.Handler = ReadSensorData;
    PostEvent(event);
}
